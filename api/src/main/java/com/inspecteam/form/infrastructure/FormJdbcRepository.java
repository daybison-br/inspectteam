...
public class FormJdbcRepository {
    ...
    public List<FormSummary> list(UUID tenantId) {
        return jdbc.sql("""
                ...
                .query((rs, rowNum) -> new FormSummary(rs.getObject("id", UUID.class), rs.getString("name"),
                        rs.getString("description"), rs.getString("status"),
                        (Integer) rs.getObject("published_version"), (Integer) rs.getObject("draft_version"),
                        rs.getTimestamp("updated_at").toInstant())).list();
    }

    public Optional<FormVersion> findDraft(UUID tenantId, UUID formId) {
        return jdbc.sql("""
                SELECT id, version_number, definition::text definition
                  FROM form_versions
                 WHERE tenant_id = :tenantId AND form_id = :formId AND status = 'DRAFT'
                """).param("tenantId", tenantId).param("formId", formId)
    ...
    public void publish(UUID tenantId, UUID formId, FormVersion draft, UUID membershipId) {
        ...
        jdbc.sql("UPDATE forms SET status = 'PUBLISHED', updated_at = NOW() WHERE tenant_id = :tenantId AND id = :formId")
                .param("tenantId", tenantId).param("formId", formId).update();
        jdbc.sql("""
                INSERT INTO form_versions (tenant_id, form_id, version_number, definition, created_by)
